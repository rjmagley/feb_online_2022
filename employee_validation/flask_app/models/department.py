...
class Department:
    ...
    @classmethod
    def get_department_by_name(cls, data):

        query = "SELECT * FROM departments WHERE name = %(department_name)s;"
    ...
    def validate_department(data):
        ...
        # if len(data['department_name']) > 50:
        #     is_valid = False
        #     flash("Department name should be 50 characters or less.")

        # if len(data['department_name']) < 2:
        #     is_valid = False
        #     flash("Department name should be at least two characters.")

        if len(data['department_name']) < 2 or len(data['department_name']) > 50:
            is_valid = False
            flash("Department name should be from 2 to 50 characters long.")

        elif len(Department.get_department_by_name(data)) != 0:
            is_valid = False
            flash("Department name is already in use.")

        if len(data['department_location']) < 3 or len(data['department_location']) > 10:
            is_valid = False
            flash("Department location should be from 3 to 10 characters long.")
